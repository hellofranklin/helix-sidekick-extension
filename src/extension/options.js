...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function create_auth_endpoint() {
        let oauth2_url = 'https://github.com/login/oauth/authorize?client_id${CLIENT_ID}';
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message.data.includes('publish')) {
        ...
        let percentageText='';
        if (request.percentCompletion > 0) {
             percentageText = "  [" + request.percentCompletion + "%]";
        }
        progressLabel.innerText = request.statusMessage +percentageText;
        progressBar.value = request.percentCompletion

        if (request.percentCompletion === 100) {
            setTimeout(() => {
        ...
        if (!request || !request.percentCompletion || request.percentCompletion === 0 || request.percentCompletion === 100) {
